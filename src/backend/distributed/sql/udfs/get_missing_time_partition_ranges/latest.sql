...
BEGIN
    ...
    AND partition_column_type <> 'timestamptz'::regtype THEN
      SELECT EXISTS(SELECT OID FROM pg_catalog.pg_cast WHERE castsource = partition_column_type AND casttarget = 'timestamptz'::regtype) AND
             EXISTS(SELECT OID FROM pg_catalog.pg_cast WHERE castsource = 'timestamptz'::regtype AND casttarget = partition_column_type)
